...
// so it themes: set the color on the parent (e.g. `text-brand` for brand).
 ...
 * the name. Shield in currentColor; baton in the brand token (themes with the
 * chosen brand). `relay` swaps the vertical baton for the diagonal hand-off.
 */
export function ShieldMark({
  ...
  return (
    ...
    >
      ...
      {relay ? (
        <>
          <path d="M9.4 14.6 L14.6 9.4" className="stroke-brand" strokeWidth="2.4" strokeLinecap="round" />
          <circle cx="8.2" cy="15.8" r="1.5" className="fill-brand" opacity="0.4" />
          <circle cx="15.8" cy="8.2" r="1.5" className="fill-brand" />
        </>
      ) : (
        <path d="M12 7 L12 15.2" className="stroke-brand" strokeWidth="2.7" strokeLinecap="round" />
      )}
    </svg>
  );
}
...
/**
 * Solid-shield mark: the shield is filled with the brand (the brand colour); the
 * baton (node·shaft·node) is knocked out to the page background, so it stays
 * high-contrast on any brand and reads cleanly at favicon size. This is the
...
export function ShieldSolid({ size = 24, className = "", title }: MarkProps) {
  return (
    ...
    >
      {title ? <title>{title}</title> : null}
      {/* shield = brand colour */}
      <path d="M5 4 L19 4 L19 12.6 C19 18 12 21.6 12 21.6 C12 21.6 5 18 5 12.6 Z" className="fill-brand" />
...
/** Logo gold — fixed brand colour, independent of the UI accent token. */
const LOGO_GOLD = "#ffaa00";

/**
 * The real batonkeep mark — the Inkscape-authored SVG. Fill is the fixed
 * logo gold (#ffbb55), not the UI brand token, so the mark colour is stable
 * regardless of which accent variant is active.
 */
 ...
 */
export default function Logo({ wordmark = true, size = 33, className = "" }: LogoProps) {
  return (
    <span className={`inline-flex items-center gap-2 ${className}`}>
      <BatonMark size={size} title="batonkeep" />
      {wordmark ? (
        <span className="font-mono font-semibold tracking-tight text-xl text-ink">
