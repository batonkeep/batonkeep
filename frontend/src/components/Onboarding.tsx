...
// D-track: composed from ui/ primitives (Modal, Card, Input, Select, Button, StatusDot).
import { useState } from "react";
import { Cloud, KeyRound, Terminal } from "lucide-react";
import type { Credential, Mode, ProviderHealth } from "../types";
import { Badge, Button, Card, Input, Modal, Select, StatusDot } from "../ui";
    ...
    >
      <div className="space-y-3">
        ...
        <Card className="p-4">
          ...
          <pre className="mt-2 rounded-lg border border-edge bg-base/60 p-2 text-xs text-ink">make auth   # walks through claude › grok › agy › codex sign-in</pre>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {g.instances.map((p) => (
                      <span key={p.name} className="flex items-center gap-1 rounded border border-edge px-2 py-0.5 font-mono text-[11px]">
                        <StatusDot tone={p.healthy ? "ok" : "neutral"} size={6} />
                        {p.label || p.name}
          ...
          <details className="mt-3 text-xs text-muted">
            ...
            <div className="mt-2 space-y-1.5">
              ...
              <pre className="rounded-lg border border-edge bg-base/60 p-2 text-ink">make auth p=claude:work</pre>
        ...
        <Card className="p-4">
          ...
          <div className="mt-2 flex flex-col gap-2 sm:flex-row">
            {customId ? (
              ...
              <Select value={provider} onChange={(e) => setProvider(e.target.value)} className="sm:w-52">
            ...
            />
            <Button variant="primary" onClick={save} disabled={saving || !apiKey.trim() || !provider.trim()}>
